...
public class OrganizationController {
    ...
    public PageResult<RegistrationEntity> queryRegistrations(@Valid @RequestBody StatusPaginationQueryRequest request) {
        return organizationService.queryRegistrations(request.getStatus(), request.getPage(), request.getPageSize());
    }

    @Secured({Authority.ADMIN, Authority.USER})
    @PostMapping("/queryOrganizations")
    @ApiOperation(value = "查询所有已注册的组织信息")
    public PageResult<OrganizationEntity> queryOrganizations(@Valid @RequestBody OrganizationQueryRequest request) {
        return organizationService.queryOrganizations(request.getOrganizationNameKeyword(), request.getPage(), request.getPageSize());
    }
